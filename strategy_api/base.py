...
This module is part of the platform's plugin contract (`alan_trader.strategy_api`)
and must not reference any specific strategy.
"""
...
class BaseStrategy(ABC):
    ...
    Optional hooks (headless — no UI types):
      current_signal(close) -> dict | None   today's live verdict for monitors
    """
    ...
    def current_signal(self, close: pd.Series) -> Optional[dict]:
        """
        Today's live verdict from a daily close series, for headless monitors
        (signal alerts, cron checks). Return None when the strategy has no cheap
        live signal. Keys the platform reads: ``signal`` (BUY/SELL/HOLD/UNKNOWN),
        ``state``, ``price``, ``asof`` and ``detail`` (one human-readable line).
        """
        return None
