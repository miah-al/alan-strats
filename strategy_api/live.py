...
(``kind`` = call | put), or one of the multi-leg STRUCTURE_KINDS below (a straddle, an iron fly, an iron condor),
always in long-structure terms (bid = what a seller of the whole structure gets, ask = what a buyer pays).
...
Fill rows beyond the vertical's (``kind`` rest | open | add | close | cancel with direction bull | bear):
  a STRUCTURE fill carries ``struct`` (straddle | iron_fly | iron_condor:<wing>), ``direction`` long | short and
      ``legs`` = [[cp, strike, sign, price], ...] (sign +1 bought, -1 sold, in long-structure terms) so the ledger
      can book every leg;
...
#: fly (its body is their midpoint), both equal the body of a straddle, and are the two SHORT strikes of an iron
#: condor, whose kind names its wing width: "iron_condor:100" (the wings sit that far outside the short strikes)
STRUCTURE_KINDS = ("straddle", "iron_fly", "iron_condor")
...
def structure_kind(kind: str) -> tuple[str, Optional[float]]:
    """("iron_condor", 100.0) for "iron_condor:100"; (kind, None) otherwise."""
    s = str(kind)
    if ":" in s:
        base, arg = s.split(":", 1)
        try:
...
def is_structure(kind: str) -> bool:
    return structure_kind(kind)[0] in STRUCTURE_KINDS
...
def structure_legs(kind: str, k_low: float, k_high: float) -> list[tuple[str, float, int]]:
    ...
    == its body; an iron fly's body is the midpoint of its wings (long the body, short the wings); an iron condor
    "iron_condor:<wing>" is long the put at k_low and the call at k_high and short the wings <wing> points outside
    them (so its seller receives the credit and its value is bounded by the wing width)."""
    base, arg = structure_kind(kind)
    if base == "iron_condor":
        if not arg or arg <= 0:
            raise ValueError(f"an iron condor names its wing width: 'iron_condor:100', not {kind!r}")
        kp, kc = float(k_low), float(k_high)
        if kc <= kp:
            raise ValueError("an iron condor's short call strike must be above its short put strike")
        return [("P", kp, 1), ("C", kc, 1), ("P", kp - arg, -1), ("C", kc + arg, -1)]
...
def structure_intrinsic(S: float, legs) -> float:
    return sum(sign * (max(S - K, 0.0) if cp == "C" else max(K - S, 0.0)) for cp, K, sign in legs)


def structure_bound(legs) -> Optional[float]:
    """The most a LONG structure can be worth at expiry (its payoff is piecewise linear, so the maximum sits at a
    strike or at the ends); None when it is unbounded (a net long option beyond the last strike)."""
    ks = sorted({float(K) for _, K, _ in legs})
    if not ks:
        return None
    far = ks[-1] * 2.0 + 1.0
    if structure_intrinsic(far, legs) > structure_intrinsic(ks[-1], legs) + 1e-9:
        return None
    if structure_intrinsic(0.0, legs) > structure_intrinsic(ks[0], legs) + 1e-9:
        return None
    return max(structure_intrinsic(k, legs) for k in ks)
