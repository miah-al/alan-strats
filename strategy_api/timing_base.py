"""
Shared helpers for equity-timing overlays (own one liquid index when "in",
cash when "out"). The pieces common to every such strategy — data loading, the
equity simulation, and turning an in/out position series into discrete trades —
live here so a strategy file holds only its own signal rule.

Part of the platform plugin contract; references no specific strategy.
"""
