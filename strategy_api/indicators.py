"""
strategy_api/indicators.py — shared technical indicators + option pricing.
...
*exactly* — proven by the plugin's tests/test_indicators.py (numerical-
equivalence gate). The only real differences between the old copies were:
...
from alan_trader.backtest.engine import bs_price  # noqa: F401  (re-exported)
...
def compute_ivr(vix: pd.Series, window: int = 252, min_periods: int = 60) -> pd.Series:
    ...
    52-week range). Old call sites used 60 or 126; pass the matching value to
    reproduce them.
    """
...
def compute_adx(high: pd.Series, low: pd.Series, close: pd.Series,
    ...
    window). Old call sites used 20.0 or 0.0; pass the matching value to
    reproduce them.
    """
