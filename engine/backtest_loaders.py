"""
engine/backtest_loaders.py

Auxiliary-data loaders for the backtest path (the service's backtest jobs, the Dash
Backtest / Performance tabs while they exist).
...
LoaderResult is a small dataclass with two fields:
  ...
  - alert       : a headless ``LoaderAlert`` to surface to the user (e.g. "no earnings
                  data found, sync first") OR None to proceed silently. If alert is
                  non-None and is_blocking is True, the dispatcher should return it
                  instead of running the backtest. ``LoaderAlert.text()`` is its plain
                  text; a UI renders its parts (bold title, line break, code) as it likes.
...
# ── Headless alert ─────────────────────────────────────────────────────────────
# A loader's message keeps the structure the Dash page renders (a bold title, a line break,
# plain text, code) without importing Dash: each part carries its text in ``children``, the
# same attribute a Dash component uses, so anything that walks a component tree reads it too.
    ...
    def text(self) -> str:
        ...
        for part in self.children:
            if isinstance(part, str):
                out.append(part)
            elif getattr(part, "children", None):
                out.append(str(part.children))
...
class LoaderResult:
    ...
    alert:        Optional[LoaderAlert] = None
...
def load_option_snapshots(engine, ticker, fd, td, *, price_data, label, **_):
    ...
    if df.empty:
        alert = LoaderAlert([
            Strong(f"{label} requires options chain data. "), BR,
...
def load_sector_etfs(engine, ticker, fd, td, *, price_data, label,
    ...
    if len(sectors) < min_count:
        ...
        alert = LoaderAlert([
            Strong(f"{label} requires ETF basket data. "), BR,
...
def load_earnings_calendar(engine, ticker, fd, td, *, price_data, label,
    ...
    if ec_df.empty and not optional:
        alert = LoaderAlert([
            Strong(f"{label} requires earnings calendar data. "), BR,
...
def load_atm_iv(engine, ticker, fd, td, *, price_data, label, **_):
    ...
    if iv.dropna().empty:
        alert = LoaderAlert([
            Strong(f"{label} requires reconstructed option IV. "), BR,
...
def load_macro(engine, ticker, fd, td, *, price_data, label, **_):
    ...
    if macro is None or macro.empty:
        alert = LoaderAlert([
            Strong(f"{label} requires treasury-yield macro data. "), BR,
...
def load_stock_bond_iv(engine, ticker, fd, td, *, price_data, label,
    ...
    if tlt_df is None or tlt_df.empty:
        return LoaderResult(alert=LoaderAlert([
            Strong(f"{label} requires {bond_ticker} price data. "), BR,
    ...
    if iv_spy.dropna().empty or iv_tlt.dropna().empty:
        ...
        return LoaderResult(alert=LoaderAlert([
            Strong(f"{label} requires option IV for BOTH legs. "),
            BR, f"No reconstructed ATM IV found for {missing} in the selected "
...
def load_minute_bars(engine, ticker, fd, td, *, price_data, label,
    ...
    if bars is None or bars.empty:
        alert = LoaderAlert([
            Strong(f"{label} requires 1-minute bars for {sym}. "), BR,
            f"No mkt.MinuteBar rows for {sym} between {fd} and {td}. Pull them with ",
            Code(f"python -m scripts.bootstrap_market_data --intraday-only --intraday {sym}"),
...
def load_option_minute_bars(engine, ticker, fd, td, *, price_data, label,
    ...
    if bars is None or bars.empty:
        ...
        alert = LoaderAlert([
            Strong(f"{label} requires per-contract option minute bars for {sym}. "), BR,
            f"No mkt.OptionMinuteBar rows for {sym} between {fd} and {td}. Pull them with ",
            Code(f"python -m scripts.bootstrap_market_data --intraday-only --option-minutes {sym}"),
...
def load_daily_close(engine, ticker, fd, td, *, price_data, label,
    ...
    if df is None or df.empty:
        return LoaderResult(alert=LoaderAlert([
            Strong(f"{label} requires daily bars for {sym}. "), BR,
...
def run_loaders_for(slug: str, engine, ticker: str, fd, td, *, price_data):
    ...
    Returns:
      (aux_data: dict, blocking_alert: LoaderAlert | None)
