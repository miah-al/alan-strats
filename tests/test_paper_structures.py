...
def test_the_replay_provider_quotes_a_straddle_from_both_legs_prints_and_names_its_legs():
    ...
    prov.day, prov.root, prov.spread, prov.carry, prov.underlying = DAY, "NDXP", make_spread(0.5), 30, "NDX"   # the flat bracket: half a point per leg
...
def test_a_structure_replay_under_the_live_spread_model_brackets_each_leg_by_its_moneyness():
    """The conservative replay default (paper/spread_model.py, 2026-09-25) has no flat half-spread: with the spot in
    hand each leg's half-spread is read off its moneyness, and without it the value curve stands in, scaled to the
    leg count -- a structure never falls over on the missing ``h`` and is never quoted narrower than a vertical."""
    from paper.spread_model import LiveSpread
    prov = ReplayProvider.__new__(ReplayProvider)
    prov.day, prov.root, prov.spread, prov.carry, prov.underlying = DAY, "NDXP", make_spread(None), 0, "NDX"
    ...
    assert q3 is not None and q3.last == 275.0 and q3.ask - q3.bid == pytest.approx(2 * LiveSpread().by_value(275.0, 50.0))
